...
| SCRIPT | references/scriptwriter.md | `script_outline.md` + `visual_plan.json` + `game/scene/*.txt` | `validate_script.py` 与 `validate_visual_plan.py` 均零错误 |
| ASSETS | references/art-director.md | 按 `visual_plan.json` 生成情绪姿态立绘/剧情动作立绘/背景状态/CG/Seedance 2.5 结局短片 + `manifest.json` | `check_assets.py` 通过（含每段结局视频的 Seedance 回执）+ **用户确认立绘像原图且姿态语义与文本一致** |
...
5. 所有资产按 art-director.md 的**竖构图规范**生成（背景/CG 9:16；立绘必须为 1024×1536 的 2:3 PNG，人物统一半身至大腿近景 `thigh_up`）。立绘禁止全身、膝盖、脚和远景；必须展示腿脚、行走或完整空间关系的动作改用 CG/画外演出。所有立绘生图必须给 `gen_image.py --asset-type figure`，不得绕过构图 Gate。
...
9. **视觉语义先于生图，但文字必须先解释变化**：所有可见动作、人物进退场、通讯离场、灯光/天气/空间状态变化先写进 `visual_plan.json`。除场景建立镜头外，使用“可见 lead_text → 220ms 内短转场 → 承接文本”的顺序，禁止图片先出现、对应文字后加载。对话姿态要有身体语言，但必须遵守最短持有行数与切图预算；禁止每个选项都机械换图，也禁止把全部差分冻结成同一姿势。
10. **结局短片强制走 Seedance 2.5**：每个被剧本 `playVideo` 引用的结局 MP4 必须由 `scripts/gen_video.py` 调用 `tools/providers.yaml` 中标记为 `seedance-2.5` 的 `seedance` provider 生成，并自动落档 `provider/model_family/model/task_id/prompt_file/reference_image/receipt`。禁止用 ffmpeg、静态图 Ken Burns、CSS 动画、手工拷贝 MP4 或其他本地合成替代；Seedance 建任务、轮询或下载失败时，ASSETS 阶段保持失败并向用户报告真实错误，**不得自行降级**。
...
projects/<id>/
├── gdd.md              # INTAKE 产出
├── script_outline.md   # SCRIPT 产出：章节、分支与数值账本
├── visual_plan.json    # SCRIPT 产出：逐视觉节拍、人物在离场状态、资产语义目录
├── effects.css         # 粒子动效设计（BUILD 产出：按主题从零推导编写，apply_theme 注入双端）
├── state.json          # 流水线状态（init_project.py 维护）
├── character/          # 用户上传的角色图、wiki、提取的角色卡
├── storyboards/        # 每个结局的 Seedance prompt
├── video_receipts/     # gen_video.py 自动写入的 Seedance 任务回执
...
| `scripts/validate_visual_plan.py <visual_plan.json> <scene目录>` | SCRIPT Gate：检查具体动作/灯光/离场叙述、文字引导顺序、短转场、姿态持有时间与每章切图预算 |
| `scripts/gen_image.py` | 生图（provider 可配，含重试）；立绘用 `--asset-type figure` 强制提示词、尺寸与人物占幅 Gate |
| `scripts/gen_video.py` | 强制调用 Seedance 2.5 生成结局短片，并自动写入 manifest 来源字段与任务回执 |
| `scripts/check_assets.py <game目录> <manifest>` | 剧本引用、立绘透明通道、1024×1536 尺寸、人物占幅与 `thigh_up` 构图契约 |
...
- 要写/改剧本 → 读 `references/scriptwriter.md`（含视觉节拍契约、演出状态机与分支回溯规范）
