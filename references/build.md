...
**同 IP / 同骨架对仗（多项目共存时的强制推导）**：当本工作区已有其他项目、或本项目是同 IP 多版本（如自由/孤独双版），封面意象、HUD 参数、粒子意象**必须逐参数与既有项目拉开差异**，差异点写进 GDD（"自由版=琥珀霓虹喧嚣 vs 孤独版=冷青细宋独处"）。禁止出现：同一骨架主题用默认参数（两个游戏 HUD 撞色）、封面同一构图公式同一意象（都是空旷夜街）、粒子同一意象（都是光斑）。QA 时同骨架/同 IP 项目**并排截图对比**，一眼分不清 = 回炉。**资产原创性由 check_assets.py 把关**：粒子 tile 撞引擎默认件、tile 命名非 `title_particles.png`（别名不生效）、背景/BGM 与其他项目 md5 碰撞，一律报错拦截；立绘跨项目复用为 WARN（同角色一致性合法，但需确认有意为之）。**唯一豁免**：同 IP 双版本（同一故事的两种呈现）可共用背景/立绘，但必须在项目根放 `.asset_share_allow`（每行一个允许共用的兄弟项目 id）显式声明，未声明即撞车按复用偷懒报错。
