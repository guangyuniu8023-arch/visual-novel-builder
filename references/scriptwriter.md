...
```
:（终章收尾旁白）;
jumpLabel:card_a -when=peace>=18;   # 与结局判定用同一变量，每个结局一张卡
jumpLabel:card_b;
label:card_a;
changeBg:cg_card_a1.jpg -enter=enter -enterDuration=1200;   # 帧 1：无字场帧（情绪前调）
changeBg:cg_card_a.jpg  -enter=enter -enterDuration=2500;   # 帧 2：烘焙标题卡面帧（双帧交叉淡入=准动画）
:（判词，无名牌旁白）;
choose:回到标题:back_a;             # 单按钮引导，卡的收尾动作
label:back_a;
end;
```

**镜头语言分结局方向**（多结局项目必须拉开，写进 GDD）：圆满=推近/转暖转亮（靠近、获得）；坏结局=拉远/降饱和（失去、抽离）；普通结局=平移/低对比（怅然）。帧差分在 CG 层面实现（帧 1→帧 2 的光线与色调变化），不依赖引擎动画。
