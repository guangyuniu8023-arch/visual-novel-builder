...
**卡面标识**：只用**结局名**（烘焙进 CG），禁止 `ENDING x/N` 编号——用户验收："02/02 这种很难理解，就叫结局的名字就好"。收集进度的呈现留给未来的图鉴页，不塞进卡面。
...
| 镜 2 | 3.5-4.5s | **角色出镜镜**（立绘合成进场景，见 art-director.md 第 8 节；结算片作为独立作品**必须至少 1 镜有角色**——纯景物轮播不像作品） | 缓推 | 人——故事的主角 |
| 镜 3 | 4-5s | 卡面 CG（晨阴→晴光双帧接力 + 判词字幕） | 推近（圆满）/平移（普通）/拉远（坏结局） | 顶点——结局名浮现 |
| 片尾卡 | 2.5-3s | 作品卡（游戏名 + tagline，PIL 制；**不要编号**） | 静止微推 | 句号——独立作品感 |

分镜从结局情绪推导（不是固定模板）：治愈=夜→人→晴的"三段收束"；悬疑=证物闪回+角色对峙；燃=高潮帧前置。镜 1/镜 2 资产优先复用项目已有背景/立绘（几乎零新增生图成本），只有镜 3 卡面必须新产。BGM：结算片必须配乐（免版税库，授权注明进 GDD；后续接生成式音乐 API），ffmpeg 混音（afade 首尾淡入淡出、volume 0.7-0.8 压人声位）。
